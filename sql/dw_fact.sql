--SELECT o.order_id, order_line = 1
--FROM dsa.orders as o
--JOIN dw.DIM_warehouse as wa ON wa.
USE DW_GlobalTech
GO
...
SELECT dsa_ord.order_id, order_line = 1, ISNULL(dim_war.sk_warehouse, 0) sk_warehouse , dim_pro.sk_product, dim_cust.bk_customer, 11111111 as sk_date, ISNULL(dim_emp.sk_employee, 0) , 
dsa_ord.[status],dsa_prod.standard_cost, dsa_ite.unit_price, dsa_ite.quantity, dsa_ite.unit_price * dsa_ite.quantity AS total_amount, (dsa_ite.unit_price - dsa_prod.standard_cost) * dsa_ite.quantity AS profit
...
SELECT dim_pro.sk_product,dim_cust.sk_customer,11111111 as sk_date, dim_emp.sk_employee, ISNULL(dim_war.bk_warehouse, 0),dsa_ord.order_id, 
CASE WHEN dsa_ord.shipped_date IS NULL THEN N'Pending' ELSE N'Shipped' END AS last_status, 
dsa_excel.was_received, DATEDIFF(DAY, dsa_ord.order_date, dsa_excel.delivery_date) AS n_days_until_cancelation
FROM dsa.orders as dsa_ord
...
SELECT dim_cust.sk_customer, 11 AS sk_payment_date, 11 AS sk_order_date, 11 AS sk_limit_payment_date, dim_emp.sk_employee, dsa_payM.method AS payment_method, dsa_ord.order_id, 
dsa_pay.payment_id, dsa_ord.[status] AS order_satus, 
CASE WHEN dsa_ite.unit_price * dsa_ite.quantity <= 100 THEN 1 ELSE 0 END AS payment_order_completed, dsa_pay.[value], 
1 AS paid_amount
